...
until sudo apt -y install python3 python3-pip python3-ujson \
        ...
        ttf-mscorefonts-installer fonts-noto fonts-roboto fonts-open-sans python3-w3lib python3-pandas python3-shapely python3-pyclipper
do
    sleep 1
done
...
if [ "${AGENT_MODE,,}" == 'desktop' ]; then
    if [ "${LINUX_DISTRO}" == 'Raspbian' ]; then
        ...
        if [ "${WPT_VB,,}" == 'y' ]; then
            vbVersionFile="viasat-browser-stable_120.0.6099.23411-1_amd64.deb"
